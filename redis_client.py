...
def get_client(db=0, a_sync=False):
    if a_sync:
        return aioredis.from_url('unix:///var/run/redis/redis.sock', decode_responses=True)
    # return redis.Redis(connection_pool=redis_pool)
    return FRedis(unix_socket_path='/var/run/redis/redis.sock', db=db, decode_responses=True)
