...
def first_triangle_order(trading_currency, trading_amount, symbol, assets, best_prices):
    ...
    if quote_asset == trading_currency:
        ...
        # if diff > 100:
        #     return
        to_amount = convert_to_base_amount(
            symbol, trading_amount, best_ask
        )  # hard coded as USDT
        # order = self.client.create_limit_buy_order(symbol, to_amount, best_ask)
        order = client.create_order("BUY", "MARKET", symbol, to_amount)
        logger.info(
            f"Market buy order create for {symbol} on base {to_amount}: {order}"
        )
        ...
        logger.info(
            f"Market sell order create for {symbol} on quote {to_amount}: {order}"
        )
