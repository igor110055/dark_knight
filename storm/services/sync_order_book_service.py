...
class SyncOrderBookService:
    ...
    def update_order_book(self, message: str, from_cache=False, last_sequence=None):
        ...
        last_sequence = last_sequence or int(self.redis.hget("last_sequences", symbol))
    ...
    def consolidate_order_book(self, response, order_book, order_book_ob):
        if order_book["bids"] and order_book["asks"]:
            ...
            if best_bid > best_ask:
                ...
                if (
                    best_prices
                    and best_prices["bids"] == best_bid
                    and best_prices["asks"] == best_ask
                ):
                    return
                logger.info(
    ...
    def get_order_book_snapshot(self, symbol, sync=False):
        if sync:
            ...
            self.redis.publish(
                "order_book_websocket",
                json.dumps({"type": "retrieve", "symbol": symbol}),
            )
            ...
            if lock.lock_acquired:
                ...
                for raw_response in responses:
                    ...
                    if not self.redis.hget("initialized", symbol):
                        if not self.has_order_book_initialized(
                ...
                self.redis.delete("cached_responses:" + symbol)
